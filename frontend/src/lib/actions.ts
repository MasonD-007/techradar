...
type CreateTechnologyRequest = components['schemas']['handlers.CreateTechnologyRequest'];
type UpdateTechnologyRequest = components['schemas']['handlers.UpdateTechnologyRequest'];
...
export async function getBlips(): Promise<Blip[]> {
    ...
    try {
        const result = await apiClient.GET('/blips') as any as { data?: Blip[]; response: Response };
...
export async function getBlip(id: number): Promise<ActionResult<Blip>> {
    ...
    try {
        const result = await apiClient.GET(`/blips/${id}`, {}) as any as { data?: Blip; response: Response };
        const { data, response } = result;
...
export async function createBlip(formData: FormData): Promise<ActionResult<Blip>> {
    ...
    try {
        ...
        const result = await apiClient.POST('/blips', { body }) as any as { data?: Blip; response: Response };
        const { data, response } = result;
...
export async function updateBlip(id: number, formData: FormData): Promise<ActionResult<Blip>> {
    const context = formData.get('context') as string;
    ...
    try {
        ...
        const result = await apiClient.PUT(`/blips/${id}`, { body }) as any as { data?: Blip; response: Response };
        const { data, response } = result;
...
export async function deleteBlip(id: number): Promise<ActionResult> {
    ...
    try {
        const result = await apiClient.DELETE(`/blips/${id}`, {}) as any as { response: Response };
...
export async function getTechnologies(): Promise<Technology[]> {
    ...
    try {
        const result = await apiClient.GET('/technologies', {}) as any as { data?: Technology[]; response: Response };
        const { data, response } = result;
...
export async function getTechnology(id: string): Promise<ActionResult<Technology>> {
    logInfo('getTechnology', 'START', { id });

    try {
        const result = await apiClient.GET(`/technologies/${id}`, {}) as any as { data?: Technology; response: Response };
        const { data, response } = result;
...
export async function getTechnologiesByQuadrant(quadrantId: number): Promise<ActionResult<Technology[]>> {
    logInfo('getTechnologiesByQuadrant', 'START', { quadrantId });

    try {
        const result = await apiClient.GET(`/technologies/by-quadrant/${quadrantId}`, {}) as any as { data?: Technology[]; response: Response };
        const { data, response } = result;

        if (!response.ok) {
            const msg = getErrorMessage(data);
            logError('getTechnologiesByQuadrant', 'ERROR', msg, { quadrantId, status: response.status });
            return { success: false, error: msg };
        }

        logInfo('getTechnologiesByQuadrant', 'SUCCESS', { quadrantId, count: data?.length || 0 });
        return { success: true, data };
    } catch (error) {
        logError('getTechnologiesByQuadrant', 'ERROR', getErrorMessage(error), { quadrantId });
        return { success: false, error: 'Failed to fetch technologies' };
    }
}

export async function createTechnology(formData: FormData): Promise<ActionResult<Technology>> {
    ...
    try {
        ...
        if (blipId) {
            ...
            const blipBody: CreateBlipRequest = { context: { created_from: 'auto-technology' } };
            const blipResult = await apiClient.POST('/blips', { body: blipBody }) as any as { data?: Blip; response: Response };
            if (!blipResult.response.ok || !blipResult.data) {
                const msg = getErrorMessage(blipResult.data);
                logError('createTechnology', 'ERROR', `Failed to create blip: ${msg}`, {});
                return { success: false, error: 'Failed to create blip' };
            }
        ...
        const result = await apiClient.POST('/technologies', { body }) as any as { data?: Technology; response: Response };
        const { data, response } = result;
...
export async function updateTechnology(id: string, formData: FormData): Promise<ActionResult<Technology>> {
    const name = formData.get('name') as string;
    const quadrantId = formData.get('quadrant_id') as string;
    const blipId = formData.get('blip_id') as string;
    ...
    try {
        ...
        const result = await apiClient.PUT(`/technologies/${id}`, { body }) as any as { data?: Technology; response: Response };
        const { data, response } = result;

        if (!response.ok) {
            const msg = getErrorMessage(data);
            logError('updateTechnology', 'ERROR', msg, { id, status: response.status });
            return { success: false, error: msg };
        }
...
export async function deleteTechnology(id: string): Promise<ActionResult> {
    ...
    try {
        const result = await apiClient.DELETE(`/technologies/${id}`, {}) as any as { response: Response };
        const { response } = result;

        if (!response.ok) {
            const msg = getErrorMessage(null);
            logError('deleteTechnology', 'ERROR', msg, { id, status: response.status });
            return { success: false, error: msg };
        }
...
export async function getUsers(): Promise<User[]> {
    ...
    try {
        const result = await apiClient.GET('/users', {}) as any as { data?: User[]; response: Response };
        const { data, response } = result;
...
export async function getUser(id: string): Promise<ActionResult<User>> {
    ...
    try {
        const result = await apiClient.GET(`/users/${id}`, {}) as any as { data?: User; response: Response };
        const { data, response } = result;
...
export async function createUser(formData: FormData): Promise<ActionResult<User>> {
    ...
    try {
        ...
        const result = await apiClient.POST('/users', { body }) as any as { data?: User; response: Response };
        const { data, response } = result;
...
export async function updateUser(id: string, formData: FormData): Promise<ActionResult<User>> {
    const name = formData.get('name') as string;
    const email = formData.get('email') as string;
    const username = formData.get('username') as string;
    const password = formData.get('password') as string;
    ...
    try {
        ...
        const result = await apiClient.PUT(`/users/${id}`, { body }) as any as { data?: User; response: Response };
        const { data, response } = result;
...
export async function deleteUser(id: string): Promise<ActionResult> {
    ...
    try {
        const result = await apiClient.DELETE(`/users/${id}`, {}) as any as { response: Response };
        const { response } = result;
