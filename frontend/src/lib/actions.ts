...
type UpdateUserTechnologyRequest = {
	user_id: string;
	technology_id: string;
	ring_id: number;
};
...
export async function getTechnologiesByQuadrant(
	...
	try {
		const result = (await api.GET(
			"/technologies/by-quadrant/{quadrant_id}" as any,
			{
...
export async function createTechnology(
	...
	if (!name || !description || !quadrantId) {
		return {
			success: false,
			error: "Name, description, and quadrant_id are required",
		};
	}
	...
	try {
		...
		const result = (await api.POST("/technologies" as any, {
			body,
		})) as any as {
			data?: Technology;
			response: Response;
...
export async function updateTechnologyInUser(
	userTechnologyId: string,
	userId: string,
	technologyId: string,
	ringId: number,
): Promise<ActionResult<UserTechnology>> {
	logInfo("updateTechnologyInUser", "START", {
		userTechnologyId,
		userId,
		technologyId,
		ringId,
	...
	if (!userTechnologyId || !userId || !technologyId) {
		return {
			success: false,
			error: "userTechnologyId, userId and technologyId are required",
		};
	}

	try {
		const body: UpdateUserTechnologyRequest = {
			user_id: userId,
			technology_id: technologyId,
			ring_id: ringId,
		};
		const result = (await api.PUT("/user-technologies/{id}" as any, {
			body,
		...
		if (!response.ok) {
			const msg = getErrorMessage(data);
			logError("updateTechnologyInUser", "ERROR", msg, {
				userTechnologyId,
				userId,
				technologyId,
				status: response.status,
		...
		logInfo("updateTechnologyInUser", "SUCCESS", {
			userTechnologyId,
			userId,
			technologyId,
			id: data?.id,
		...
		logError("updateTechnologyInUser", "ERROR", getErrorMessage(error), {
			userTechnologyId,
			userId,
			technologyId,
		});
		return { success: false, error: "Failed to update technology ring" };
	}
}
...
export async function register(
	...
	try {
		...
		const result = (await api.POST("/auth/register" as any, {
			body,
		})) as any as {
			data?: AuthResponse;
			response: Response;
...
export async function getCurrentUser(): Promise<User | null> {
	...
	try {
		...
		const { data, response } = result;
		//console.log("getCurrentUser API response:", { data, status: response.status });
